...
Diagram Description: When the user types in their correct user name in password to log into the application, first the server validates the input, then it finds the user with the matching email in the database and gets the user’s information. Next the server checks if the user entered the correct password. Since the user entered the correct password the server returns a JWT which the browser stores in local storage. Then the server gets the current user’s information and returns the current user’s information to the client. Then the server gets all posts and comments from the database and returns all posts. The newest posts are displayed in the “New” tab of the home tab. When a user clicks like on a post, the server finds the post and if the user has already liked this post. Since the user has not already liked this post the server adds this user to this post’s likes in the database. The server returns that the post has been liked and the updated like count for the post. The updated like button and like count is displayed on the post. When a user creates a new post, a new post is created on the database. Then the server gets all posts and comments from the database and returns all posts. The newest posts are displayed in the “New” tab of the home tab.
